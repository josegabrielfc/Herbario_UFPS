...
export const usePlantImgStatusForm = () => {
  ...
  const handleStatusToggle = async () => {
    ...
    try {
      await Services.plantImages.toggleStatus(selectedImageId);
      setSuccess(true);
      ...
      alert(`La imagen ha sido ${!selectedImage.status ? 'activada' : 'desactivada'}`);
    } catch (err) {
