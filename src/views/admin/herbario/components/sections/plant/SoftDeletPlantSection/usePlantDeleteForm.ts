...
export const usePlantDeleteForm = () => {
  ...
  const handleDeleteToggle = async () => {
    ...
    try {
      await Services.plants.softDelete(parseInt(selectedPlantId));
      setSuccess(true);
      await refreshAll({
      ...
      alert(`La planta "${plant.common_name}" ha sido ${!plant.is_deleted ? 'eliminada' : 'restaurada'}`);
    } catch (err) {
